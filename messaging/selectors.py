...
def messages_for_user(user): return WhatsAppMessage.objects.filter(business__owner=user).select_related("invoice", "business")
...
def filter_messages(queryset, params):
    ...
    if params.get("status"): queryset = queryset.filter(delivery_status=params["status"])
